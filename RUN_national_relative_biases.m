...
if 0,
    ...
    [mask,~,~] = CDF_land_mask(5,1,5,0);
    mask = ~mask';
    [SST_ref,~,~] = re_function_general_regional_average(SST_ref,[-57.5:5:57.5],mask(:,7:30));
end
SST_ref = squeeze(nanmean(nanmean(SST_ref,1),2));
...
for bucket_size = [0.5:0.1:2]
    ...
    if 0,
        ...
        [SST_exp,~,~] = re_function_general_regional_average(SST_exp,[-57.5:5:57.5],mask(:,7:30));
    end
    SST_exp = squeeze(nanmean(nanmean(SST_exp,1),2));
...
pic = SST_save(:,2:end) - SST_ref(end);
...
CDF_panel([0 15 0.5 2],'',{},'On-deck time (minutes)','Relative bucket size');
clear('h')
col = jet(7)*.92;
...
nation_list = [0.01 0.02 0.03 -0.06 0.10 -0.10 -0.13];
for i = 1:7
...
CDF_save(1,'png',500,['/Users/zen/Dropbox/Research/SST_bucket_regional_bias/',...
    'SST_20180420_Method/Figure8/20180708_Bucket_model_case_1.png']);
