...
% [SST_out] = BKT_canvas_model(E,P,PP)

function [SST_out] = BKT_canvas_model(E,P,PP)

    true_SST      = E.true_SST;
    true_AT       = E.true_AT;
    e_air         = E.e_air;
    u_environment = E.u_environment;
    Cs            = E.Qs;
    direct_ratio  = E.direct_ratio;
    zenith_angle  = E.zenith_angle;
    ...
    cp_bucket         = P.cp_bucket;
    leakage_rate_haul = P.leakage_rate_haul;  % unit: m/min
    leakage_rate_deck = P.leakage_rate_deck;
    diameter          = P.diameter;
    depth             = P.depth;
    albedo_bucket     = P.albedo_bucket;
    ...
    % Term Management -----------------------------------------------------
    sensible_on   = PP.sensible_on;
    latent_on     = PP.latent_on;
    long_on       = PP.long_on;
    solar_top_on  = PP.solar_top_on;
    solar_wall_on = PP.solar_wall_on;
    ...
    if 1
        ...
        nanmean(h_cycle_haul(:));
    else
        pow = 2;
        scl = 50;
        h_cycle_haul = 2.8 * (u0_haul./diameter).^pow /scl;
        h_cycle_deck = 2.8 * (u0_deck./diameter).^pow /scl;
        h_base_haul  = 4.3 * (u0_haul./diameter).^pow /scl;
        h_base_deck  = 4.3 * (u0_deck./diameter).^pow /scl;
        nanmean(h_cycle_haul(:))
    end
